...
class Train_info_record_sw_emb:
    ...
    def update_cur_train_info(self,args):    
        ...
        train_log.write(f"dataset = {args.dataset}, dim = {args.dim}, batch_size = {args.batch_size}, use_nb_rate = {args.use_neighbor_rate}, l2_weight = {args.l2_weight}\n")
...
class Train_info_record_emb_sw_ndcg:
    ...
    def update_score(self, step, eval_score_info):
        ...
        train_log.write(f"step = {step}, eval pre = {eval_precision} ")
        train_log.write(f"{step}, test pre = {test_precision} \n")
        train_log.close()
    ...
    def record_final_score(self):
        ...
        for tags_label in range(len(self.tags)):
            ...
            train_log.write(f"{self.tags[tags_label]}{space} = {emb_score_ndcg[tags_label]}, recall = {emb_score_recall[tags_label]}, precision = {emb_score_precision[tags_label]} \n")
        train_log.write(f"{'*'*120} \n")
        train_log.close()
