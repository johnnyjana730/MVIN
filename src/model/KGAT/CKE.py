...
class CKE(object):
    ...
    def _build_model(self):
        ...
        self.u_e, self.i_e = self._get_cf_rating_inference(self.user_indices, self.item_indices)
    ...
    def _get_cf_rating_inference(self, user, item):

        u_e = tf.nn.embedding_lookup(self.weights['user_embed'], user)
        i_e = tf.nn.embedding_lookup(self.weights['item_embed'], item)
        # neg_i_e = tf.nn.embedding_lookup(self.weights['item_embed'], neg_i)

        i_kg_e = tf.reshape(tf.nn.embedding_lookup(self.weights['kg_entity_embed'], item), [-1, self.emb_dim])
        # neg_i_kg_e = tf.reshape(tf.nn.embedding_lookup(self.weights['kg_entity_embed'], neg_i), [-1, self.emb_dim])

        return u_e, i_e + i_kg_e
    ...
    def _get_cf_loss(self):
        ...
        cf_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        ...
        cf_reg_loss = tf.nn.l2_loss(self.u_e) + tf.nn.l2_loss(self.i_e)
