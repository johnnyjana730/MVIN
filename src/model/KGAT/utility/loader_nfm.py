...
class NFM_loader(Data):
    ...
    def generate_rating_batch(self, data ,start, end):
        ...
        u_sp = self.user_one_hot[data[start:end, 0]]
        i_sp = self.kg_feat_mat[data[start:end, 1]]

        i_feats = sp.hstack([u_sp, i_sp])

        batch_data = {}
        batch_data['item_feats'] = i_feats
    ...
    def generate_feed_rating_dict(self, model, batch_data):
        ...
        i_indices, i_values, i_shape = self._extract_sp_info(batch_data['item_feats'])
